...
class RANSAC:
    ...
    def create_sample_dict(
        self, image, num_instances=None, best_rect=None, mask=None, best_inlier_ratio=0
    ):
        ...
        if best_rect is not None and mask is not None:
            ...
            points = [
                point
                for point in points
                if np.sqrt((point[0] - center[1]) ** 2 + (point[1] - center[0]) ** 2)
                < 50
            ]
    ...
    def sample_params(self, params: dict = {}, mode: str = "coarse"):
        ...
        if mode == "coarse":
            ...
            theta_idx = np.random.choice(
                params["theta_array"].shape[0], 1, replace=False
            )
            theta = params["theta_array"][theta_idx]
            point = params["points"][point_idx].squeeze(0)
            l = np.random.uniform(
                (1 - params["l_threshold"]) * params["l_mean"],
                (1 + params["l_threshold"]) * params["l_mean"],
            )
            w = np.random.uniform(
                (1 - params["w_threshold"]) * params["w_mean"],
                (1 + params["w_threshold"]) * params["w_mean"],
            )
            ...
            sampled_pointx = np.random.uniform(
                low=best_rect[0][1] - 10, high=best_rect[0][1] + 10
            )
            sampled_pointy = np.random.uniform(
                low=best_rect[0][0] - 10, high=best_rect[0][0] + 10
            )
    ...
    def ransac_rect(
        ...
        best_area = (
            best_rect[1][0] * best_rect[1][1]
            if params["best_rect"] is not None
            else None
        )
        ...
        while iteration < params["num_iterations"] and not found:
            ...
            sampled_point, sampled_theta, sampled_l, sampled_w = self.sample_params(
                params=params, mode=mode
            )

            # Define the center and size of the rotated rectangle
            center = (int(sampled_point[1]), int(sampled_point[0]))
            size = (sampled_w, sampled_l)
            angle = int(sampled_theta[0] * 180 / np.pi)
            ...
            if log:
                ...
                plot(
                    log_image,
                    title=f"{mode} Ransac: Depth image, detection, intersection",
                )
    ...
    def publish_result(self,centroid,quat):
        ...
        self._result.pose.header.frame_id = "grasp_pose"
    ...
    def inference(self, color, depth):
        ...
        # preprocess image
        color_img_, depth_img_, depth_mask = preprocess_image(
            color_img,
            depth_img,
        ...
        #visualize color and depth image
        # cv2.imshow("color", color_img_)
        # cv2.imshow("depth_mask", depth_mask)
        # cv2.waitKey(0)

        # plot(depth_img_,type="colormap", title="Depth image after preprocessing")
        ...
        res = self.ransac_rect(
            color_img_, depth_mask, num_instances, mode="coarse", log=self.log
        )
        ...
        params = self.create_sample_dict(
            depth_mask,
            num_instances,
            best_rect=res["best_rect"],
            mask=res["mask"],
            best_inlier_ratio=res["best_inlier_ratio"],
        )
        res = self.ransac_rect(
            color_img_, depth_mask, num_instances, params, mode="fine", log=self.log
        )
        ...
        #convert roll,pitch,yaw to quaternion using scipy
