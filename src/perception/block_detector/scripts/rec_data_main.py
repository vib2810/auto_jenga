...
class RANSAC():
  ...
  def inference(self,color,depth):
    ...
    #plot color_image
    # plot(color_img_,title="Color image") 

    #plot depth_mask
    # plot(depth_mask,title="Depth mask")

    color_hsv_mask, hsv_image_mask = compute_hsv_mask(color_img_,lower=(9, 20, 20),upper= (16, 100, 255))
    # plot(hsv_image_mask,title="HSV mask")
    ...
    depth_mask_[idx[0]+self.bounds[0],idx[1]+self.bounds[2]] = 1
    ...
    if(self.log):
      plot_pcd(pcd_cropped,centroid=centroid,angle=(0,0,yaw))
